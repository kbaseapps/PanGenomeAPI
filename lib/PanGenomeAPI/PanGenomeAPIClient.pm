...
=head2 compute_summary_from_pangenome

  $result = $obj->compute_summary_from_pangenome($params)
...
<pre>
$params is a PanGenomeAPI.ComputeSummaryFromPG
$result is a PanGenomeAPI.ComputeSummaryFromPGResult
ComputeSummaryFromPG is a reference to a hash where the following keys are defined:
	pangenome_ref has a value which is a string
ComputeSummaryFromPGResult is a reference to a hash where the following keys are defined:
	families has a value which is a reference to a hash where the key is a string and the value is an int
	genes has a value which is a reference to a hash where the key is a string and the value is an int
	shared_family_map has a value which is a reference to a hash where the key is a string and the value is a reference to a hash where the key is a string and the value is an int
	genome_ref_name_map has a value which is a reference to a hash where the key is a string and the value is a reference to a hash where the key is a string and the value is a string
	pangenome_id has a value which is a string
	genomes has a value which is an int
...
=cut
 ...
 sub compute_summary_from_pangenome
{
    my($self, @args) = @_;
...
# Authentication: optional
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function compute_summary_from_pangenome (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to compute_summary_from_pangenome:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'compute_summary_from_pangenome');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "PanGenomeAPI.compute_summary_from_pangenome",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method compute_summary_from_pangenome",
					    status_line => $self->{client}->status_line,
					    method_name => 'compute_summary_from_pangenome',
				       );
    }
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method compute_summary_from_pangenome",
            status_line => $self->{client}->status_line,
            method_name => 'compute_summary_from_pangenome',
        );
    }
...
=head2 ComputeSummaryFromPGResult
